...
store.complete('2:1',{reservation:reserved.reservation,catalogue});assert.equal(store.reserve('2:1','1','catalogue').reason,'already-completed');clock+=5100;
...
clock+=5100;assert.equal(store.reserve('3:1','1').reason,'nothing-due');
...
const {scannerOperationalStatus}=await import('../worker/mutual-funds-scanner-model.mjs');
const operational=scannerOperationalStatus({expectedCompanies:2,currentCompanies:1,cooldownUntil:clock+60000,companies:[{isin,checkedAt:new Date(clock).toISOString(),state:'ok'},{isin:newIsin,state:'http-429'}]},clock);
assert.deepEqual(operational.states,{ok:1,'http-429':1});assert(!JSON.stringify(operational).includes(isin));assert(!('companies' in operational));
assert.match(supplementStatus({supplement:{...operational,companies:[]}},clock),/Source paused until/);assert(!supplementStatus({supplement:{...operational,companies:[]}},clock).includes('paused until Not checked'));
const {authoriseMutualFundsReader}=await import('../worker/mutual-funds-auth.mjs');
const readerRequest=new Request(MF_ORIGIN+'/api/mutual-funds/private',{headers:{authorization:'Bearer fixture-reader-token'}});
const profile=async()=>Response.json({email:'reader@example.com'});
...
assert.match(supplementStatus({supplementAccess:'configuration-unavailable'}),/access setup unavailable/);
console.log('PASS MF-only verified reader policy, distinct identity failures and aggregate-only source diagnostics');
let deniedFetches=0;
const deniedRun=await collectScanner({companies:[{isin,name:'Fixture Bank'}],client:async b=>b.action==='scanner-reserve'?(b.kind==='catalogue'?{catalogue}:{reservation:'99:1:2',company:{isin,name:'Fixture Bank'},url}):{ok:true},fetcher:async()=>{deniedFetches++;return new Response('',{status:429,headers:{'retry-after':'7200'}});}});
assert.equal(deniedFetches,1);assert.equal(deniedRun.reason,'http-429');assert.deepEqual(deniedRun.failures,{'http-429':1});
assert.equal(deniedRun.failed,1);assert.equal(deniedRun.completed,0);
console.log('PASS source refusal stops collection and reports its failure category without private observations');
