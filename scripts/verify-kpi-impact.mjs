...
try {
  ...
  assert.equal(await kpi.load({ wait: true }), held, 'an unchanged file keeps the same ontology, so the ranking memo survives the check');
  ...
  // A paint never waits on the re-read: the held copy answers at once and the new one lands behind it.
  assert.equal(await kpi.load(), held, 'a re-read does not hold up a paint');
  assert.equal(requests, 2, 'but it has been asked for');
  const changed = await kpi.load({ wait: true });
  assert.equal(requests, 2, 'one re-read, shared by both calls');
  ...
  clock += kpi.RECHECK_MS + 1;
  refuse = true;
  assert.equal(await kpi.load({ wait: true }), changed, 'a failed re-read keeps the copy it had');
  assert.equal(kpi.status().state, 'ready');
  assert.match(kpi.status().recheckError, /could not be re-read/, 'and says the re-read failed');
  refuse = false;
  assert.equal(await kpi.load({ wait: true }), changed, 'the next check asks again at once and, finding it unchanged, keeps it');
