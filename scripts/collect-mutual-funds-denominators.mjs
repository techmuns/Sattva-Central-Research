...
for(const h of book.holdings.filter(h=>h.ticker)) {
  try {
    ...
    if(data.info?.isin===h.isin&&Number.isSafeInteger(shares)&&shares>0)denominators[h.isin]={shares,checkedAt:new Date().toISOString(),asOf:data.metadata?.lastUpdateTime||null,source:url,kind:'exchange',method:'NSE issued shares, exact ISIN match'};
  }catch{/* A failed denominator read cannot invent a value. */}
  await new Promise(done=>setTimeout(done,200));
}
fs.mkdirSync('artifacts',{recursive:true});fs.writeFileSync('artifacts/mutual-funds-denominators.json',JSON.stringify(denominators));
