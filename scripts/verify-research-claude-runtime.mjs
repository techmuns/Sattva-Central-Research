...
globalThis.fetch = async (url, options) => {
  ...
  if (outbound.url !== 'https://bedrock-runtime.ap-south-1.amazonaws.com/anthropic/v1/messages' || outbound.redirect !== 'manual' || outbound.headers.get('x-api-key') !== 'ABSK-synthetic-bedrock-runtime-key') throw new Error('Invalid native provider request');
...
export default { fetch(request) { return handleResearch(request, {CLAUDE_KEY:'ABSK-synthetic-bedrock-runtime-key'}); } };
`);
...
try {
  ...
  assert.equal((await (await fetch(origin)).json()).provider, 'bedrock');
