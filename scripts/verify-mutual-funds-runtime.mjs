...
try{
 ...
 assert.equal((await call({action:'private-read',isins:[other]})).rows[0].totalShares,75);
 assert.equal((await call({action:'read',isins:[other]})).rows[0].totalShares,null);
 await stop();await start();assert.equal((await call({action:'private-read',isins:[other]})).rows[0].totalShares,75);
