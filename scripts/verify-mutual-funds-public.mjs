...
for(const label of ['Goldiam International Limited','Silverline Technologies','Cash Systems Limited','TREPS Holdings','Grand Total Industries'])for(const id of ['', 'MALFORMED'])assert(!verifiedNonIndianRows([...cashRows,[label,id,1]],'quant Overnight Fund',month),`An unclassified company named ${label} cannot create a false exit`);
assert(!verifiedNonIndianRows([...cashRows,['Goldiam International Limited','','1']],'quant Overnight Fund',month),'Malformed weights cannot conceal an unclassified position');
