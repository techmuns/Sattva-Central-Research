...
try {
  ...
  assert.equal(await page.locator('.research-sidebar').isVisible(), false, 'history does not consume answer space by default');
  assert.equal(await page.locator('[data-beacon-toggle]').isVisible(), false, 'floating source badge never overlays research answers');
  await page.getByRole('button', { name: 'Conversation history', exact: true }).click();
  assert(await page.getByRole('dialog', { name: 'Research conversations', exact: true }).isVisible());
  assert.equal(await page.locator('[data-research-history]').getAttribute('aria-expanded'), 'true');
  await page.keyboard.press('Escape');
  assert.equal(await page.locator('.research-sidebar').isVisible(), false);
  assert(await page.locator('[data-research-history]').evaluate(node => node === document.activeElement), 'closing history restores keyboard focus');
  await page.getByRole('button', { name: 'Conversation history', exact: true }).click();
  await page.locator('.research-session.is-active [data-research-session]').click();
  assert.equal(await page.locator('.research-sidebar').isVisible(), false, 'selecting a conversation returns to the answer');
  assert.equal(await input.inputValue(), 'Keep this next question', 'history selection preserves the next draft');
  await page.getByRole('button', { name: 'Research sources', exact: true }).click();
  assert(await page.locator('#modal-container').isVisible(), 'canonical source guide remains available in the compact toolbar');
  await page.keyboard.press('Escape');
  await answerArticle.getByRole('button', { name: 'Read from start', exact: false }).click();
  const normalHeight = await page.locator('[data-research-transcript]').evaluate(node => node.clientHeight);
  assert(normalHeight >= 1050 * 0.68, 'normal desktop layout dedicates most of the viewport to the transcript');
  ...
  for (const size of [{ width: 1024, height: 768 }, { width: 390, height: 844 }, { width: 320, height: 640 }, { width: 390, height: 520 }]) {
    ...
    const layout = await page.evaluate(() => {
      ...
      return { transcriptHeight: transcript.height, bottom: composer.bottom, right: composer.right, pageHeight: document.documentElement.scrollHeight, pageWidth: document.documentElement.scrollWidth };
    });
  ...
  assert.equal(await page.locator('[data-app-nav]').isVisible(), false, 'reading view gives the dashboard navigation space to the answer');
  assert.equal(await input.inputValue(), 'Keep this next question');
  await answerArticle.getByRole('button', { name: 'Read from start', exact: false }).click();
  const needsJump = await page.locator('[data-research-transcript]').evaluate(node => node.scrollHeight - node.scrollTop - node.clientHeight >= 100);
  assert.equal(await page.locator('[data-research-latest]').isVisible(), needsJump, 'the latest-answer control appears only when more content is below');
  ...
  assert(geometry.width <= 900 && geometry.font >= 16, 'answers use a wider column with comfortably sized text');
  assert(await page.locator('[data-research-transcript]').evaluate(node => node.clientHeight >= innerHeight * 0.85), 'reading view devotes at least 85% of the frame to answers');
  ...
  if (needsJump) await page.getByRole('button', { name: 'Latest answer', exact: false }).click();
  ...
  assert.equal(await page.locator('.research-sidebar').isVisible(), false, 'leaving reading view keeps history collapsed');
  assert(await page.locator('[data-app-nav]').isVisible(), 'leaving reading view restores dashboard navigation');
  await page.getByRole('button', { name: 'Reading view', exact: true }).click();
  await input.press('Escape');
  assert(await page.locator('[data-app-nav]').isVisible(), 'Escape also restores dashboard navigation');
  assert.equal(await input.inputValue(), 'Keep this next question');
  await page.setViewportSize({ width: 1440, height: 1050 });
  customAnswer = null;
