...
const failureReason=error=>['forbidden','not-found','unauthorised'].includes(error.code)?'access-unavailable':'dispatch-unavailable';
...
export class MutualFundsSchedule {
  ...
  async scannerWake(at) {
    ...
    try {
      ...
      else if(at-completedAt(recent)<MF_INTERVAL)scanner.reason=recent.conclusion==='success'?'recent-run':'recent-failure';
      else if(scanner.dispatchRetryAt>at)scanner.reason=scanner.dispatchFailure;
      else if(at-(scanner.lastDispatchAt||0)<90000)scanner.reason='awaiting-run';
      else {
        scanner.lastDispatchAt=at;scanner.dispatchRetryAt=null;scanner.dispatchFailure=null;
    ...
    }catch(error){scanner.reason=failureReason(error);if(error.code!=='dispatch-uncertain'){scanner.dispatchFailure=scanner.reason;scanner.dispatchRetryAt=at+MF_INTERVAL;}}
    await this.storage.transaction(async tx=>{const latest=await tx.get(MF_TIMER);await tx.put(MF_TIMER,{...latest,scanner});});
  }
  async wake() {
    const at=this.now();
    ...
    let reason='dispatch-unavailable',nextAt=at+MF_INTERVAL,source=null,importSourceRun=null,importDispatchClaimed=false;
    try {
      if(!this.env.GH_DISPATCH_TOKEN || this.env.GH_REPO!=='techmuns/Sattva-Central-Research')throw Error('Configuration');
      const cfg={token:this.env.GH_DISPATCH_TOKEN,owner:'techmuns',repo:'Sattva-Central-Research',ref:'main'};
      ...
      try {
        ...
        if(isInFlight(latest)) {
        ...
        } else if(source.dispatchRetryAt>at) {
          source.reason=source.dispatchFailure;nextAt=Math.min(nextAt,source.dispatchRetryAt);
          ...
          source.lastDispatchAt=at;source.dispatchRetryAt=null;source.dispatchFailure=null;
        ...
        source.reason=failureReason(error);
        // A definitive refusal is not an accepted request awaiting run visibility.
        // Keep it visible across frequent importer wakes; retry at the normal cadence.
        if(error.code!=='dispatch-uncertain'){source.dispatchFailure=source.reason;source.dispatchRetryAt=at+MF_INTERVAL;}
      ...
      else if(state.importDispatchRetryAt>at){reason=state.importDispatchFailure;nextAt=Math.min(nextAt,state.importDispatchRetryAt);}
      else if(at-(state.importLastDispatchAt||0)<90000){reason='awaiting-run';nextAt=Math.min(nextAt,at+60000);}
      else {
        await this.storage.transaction(async tx=>{const latest=await tx.get(MF_TIMER);await tx.put(MF_TIMER,{...latest,importLastDispatchAt:at,importDispatchRetryAt:null,importDispatchFailure:null,importPendingSourceRun:completedSource?.id||null,importKnownRuns:consumerRuns.map(r=>r.id)});});
        importDispatchClaimed=true;
      ...
      if(error.code!=='dispatch-uncertain')await this.storage.transaction(async tx=>{const latest=await tx.get(MF_TIMER);await tx.put(MF_TIMER,{...latest,importDispatchRetryAt:at+MF_INTERVAL,importDispatchFailure:reason,...(importDispatchClaimed?{importPendingSourceRun:null}:{})});});
