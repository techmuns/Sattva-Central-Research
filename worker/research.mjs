...
import { claudeCredential, streamClaudeChat } from './research-claude.mjs';
...
export function researchConfigured(env) {
  if (researchProvider(env) === 'claude') return claudeCredential(env).length > 10;
  return researchToken(env).length > 10;
}

export function researchProvider(env) {
  return claudeCredential(env) ? 'claude' : 'muns';
}
