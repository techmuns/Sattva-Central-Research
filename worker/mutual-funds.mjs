...
export async function handleMutualFunds(request,env,{identity=breakoutCollectorIdentity}={}) {
  ...
  try {
    ...
    if(url.pathname==='/api/mutual-funds/health') {
      ...
      return reply({...payload.meta,schedule},payload.meta.health.state!=='current'||schedule.overdue||['dispatch-unavailable','run-overdue'].includes(schedule.reason)||['access-unavailable','dispatch-unavailable','run-overdue'].includes(schedule.source?.reason)?503:200);
    }
