...
import { readBseCollector } from './bse-ipo-collector.mjs';
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/ipo-filings') return handleIpoFilings(request, {
      readBse: ({ signal }) => readBseCollector({ token: env.GH_DISPATCH_TOKEN, signal: AbortSignal.any([signal, AbortSignal.timeout(12000)]) }),
    });
