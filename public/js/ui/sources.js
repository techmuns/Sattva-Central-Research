    ...
    ipoSourceGroup(),
    {
      ...
      items: [
        ...
        },
        {
          ...
          feeds:
            'The <strong>Deep Dive</strong> button on every ticker-mapped call row. The click itself starts a separate LLM pipeline and immediately shows its progress; there is no connection/setup screen and no service URL in the panel. <strong>The whole report is theirs</strong>; this page lays it out and computes nothing on top. The free finished-report index is read once per visit. A transcript-backed report with a confirmed quarter fills a document-only row\'s result, view and headline only when that ticker has one unambiguous call date in the quarter; otherwise the cells remain blank. <strong>No score or sentiment tier is inferred.</strong> A report you have seen is kept on this device, so reopening it needs no run and no request at all.',
