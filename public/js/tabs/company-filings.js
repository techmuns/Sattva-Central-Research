...
import { loadDomesticFilings } from '../data/domestic-filings.js';
...
export function renderCompanyFilings(ctx, { controls = '', wireControls = () => {} } = {}) {
  ...
  ctx.root.innerHTML = `${sectionHead({ title: 'Company Filings', description: 'Annual reports, earnings reports and concall transcripts from Screener.in.', controls })}
    <div class="rounded-2xl bg-white p-5 ring-1 ring-slate-200">
      <form data-document-search class="flex flex-wrap items-end gap-3">
        ...
        <button type="submit" class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-semibold text-white">Fetch filings</button>
      </form>
      <p class="mt-3 text-xs text-slate-500">Choose a company in the selected scope. Documents open at their original source.</p>
    </div>
  ...
  async function fetchDocuments() {
    ...
    try {
      const result = await loadDomesticFilings(ticker, type, { signal: active.signal });
